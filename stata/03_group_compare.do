...
* 🖼️ t검정을 그림으로 — 두 집단 분포(상자)가 겹치는지 떨어지는지 눈으로
graph box life_exp, over(ssa) ///
    title("Life expectancy: Sub-Saharan Africa vs Other") ytitle("Life expectancy (years)")
graph export "03_box_ssa.png", replace width(1200)
...
* 🖼️ ANOVA를 그림으로 — 소득그룹별 분포(상자)가 위로 갈수록 길어진다
graph box life_exp, over(income_n, label(angle(20))) ///
    title("Life expectancy by income group (ANOVA)") ytitle("Life expectancy (years)")
graph export "03_box_income.png", replace width(1200)
display as text "   {bf:→} 저장된 PNG: 03_box_ssa.png · 03_box_income.png (작업폴더)"
