...
* ── 🖼️ 그림으로 — Preston 곡선 + 회귀선 (base STATA graph → PNG 저장) ─────────
display _newline(1) as result "■ 그림으로 보기 — Preston 곡선 + 회귀선"
display as text          "   회귀선(빨강)을 데이터 위에 직접. '왜 log로 폈는지'가 한눈에. Graph 창에 뜨고 PNG로도 저장됩니다."
twoway (scatter life_exp log_gdp, msize(vsmall) mcolor(navy)) ///
       (lfit    life_exp log_gdp, lcolor(red) lwidth(medthick)), ///
    title("Preston curve: log(GDP) to life expectancy") ///
    xtitle("log(GDP per capita)") ytitle("Life expectancy (years)") ///
    legend(order(1 "Data" 2 "Regression line"))
graph export "04_preston.png", replace width(1200)
display as text "   {bf:→} 저장된 PNG: 04_preston.png (작업폴더)"
