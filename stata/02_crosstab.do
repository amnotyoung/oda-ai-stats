...
* ── 🖼️ 그림으로 — 소득그룹별 국가 수 (base STATA graph → PNG 저장) ───────────
display _newline(1) as result "■ 그림으로 보기 — 소득그룹별 국가 수"
display as text          "   교차표의 한 축(소득그룹)을 막대로. Graph 창에 뜨고 PNG로도 저장됩니다."
gen byte one = 1                                     // 국가 수를 세기 위한 보조 변수(행당 1, 국가당 1행)
graph bar (sum) one, over(income_name, label(angle(20))) ///
    title("Number of countries by income group") ytitle("Number of countries") blabel(bar)
graph export "02_bar_income.png", replace width(1200)
display as text "   {bf:→} 저장된 PNG: 02_bar_income.png (작업폴더)"
