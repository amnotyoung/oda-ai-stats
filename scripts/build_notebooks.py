...
work.groupby("income_name")["life_exp"]\\
    ...
    md("""## 4) 🖼️ 보기 — 분포·격차를 그림으로
표의 숫자만으론 안 보이는 **쏠림(왜도)**과 **그룹 격차**가 그림엔 바로 드러난다. *왜 로그변환을 했는지*도 그림이 답한다."""),
    code("""# 왜 로그변환을 했나? — '분포'를 그리면 한눈에 보인다(한쪽으로 쏠림 = 왜도)
...
plt.tight_layout(); plt.show()                                    # 여백 정리 후 표시"""),
    code("""# 소득그룹별 기대수명 — 표의 '평균' 한 숫자를 분포(상자)로 보면 격차가 또렷
...
pd.crosstab(countries["region_name"], countries["income_name"])"""),
    md("> 🖼️ **그림으로**: 위 교차표의 숫자를 **색의 진하기**로 — 어느 지역에 어떤 소득국이 몰렸는지 한눈에."),
    code("""# 교차표를 히트맵으로 — 숫자 표가 '패턴'으로 보인다(진할수록 국가 많음)
...
print(f"[ANOVA] 소득그룹별 기대수명  F={F:.0f}  p={p2:.1e}")"""),
    md("> 🖼️ **그림으로**: 검정이 본 '평균 차이'를 **분포(상자)**로 — 상자가 겹치는지 떨어지는지 눈으로 확인."),
    code("""# 소득그룹별 기대수명 분포 — ANOVA의 'p<0.05(그룹 간 차이 유의)'를 그림으로 확인
...
print(f"\\nR² = {m.rsquared:.3f}")  # 설명력(0~1): 소득이 기대수명 변동의 몇 %를 설명하나"""),
    md("> 🖼️ **그림으로**: 회귀선을 데이터 위에 직접 그린다 — *왜 로그변환이 필요한지*가 한눈에 보인다(Preston 곡선)."),
    code("""# Preston 곡선 — 회귀를 '직선 그리기'로 직접 본다. 로그변환 전후를 나란히 비교
...
print("\\n→ 4.59 → 3.54 → 1.26 : 무엇을 통제하느냐에 따라 답이 바뀐다(식별의 문제).")'''),
    md("> 🖼️ **그림으로**: 세 모형의 소득 계수를 **막대**로 — 통제를 더할수록 계수가 줄어드는 게 한눈에."),
    code('''# 세 모형의 '소득 계수'를 막대로 비교 — 통제를 더할수록 어떻게 변하나(식별의 문제)
import matplotlib.pyplot as plt
labels = ["통제 없음\\n(pooled)", "국가 FE", "이원 FE\\n(국가+연도)"]                 # 3개 모형
...
pd.Series(rf.feature_importances_, index=X.columns).sort_values(ascending=False).head(6).round(3)"""),
    md("> 🖼️ **그림으로**: 모델 성능(R²)과 변수 중요도를 **막대 길이**로 — 숫자만 보던 결과가 직관적으로 비교된다."),
    code("""# 모델 성능과 변수 중요도를 그림으로 — 숫자(R²·중요도)를 막대 길이로 비교
import matplotlib.pyplot as plt
fig, ax = plt.subplots(1, 2, figsize=(11, 4))
# (왼쪽) 안 본 데이터에서의 예측력: 랜덤포레스트 vs 선형회귀
names  = ["랜덤포레스트", "선형회귀(OLS)"]; scores = [rf.score(Xte, yte), ols.score(Xte, yte)]
b = ax[0].bar(names, scores, color=["#4c78a8", "#bbbbbb"])
for bar, s in zip(b, scores):                                    # 막대 위에 점수 표기
    ax[0].text(bar.get_x() + bar.get_width() / 2, s, f"{s:.3f}", ha="center", va="bottom", fontweight="bold")
ax[0].set_ylim(0, 1); ax[0].set_ylabel("test R² (예측력)"); ax[0].set_title("모델 성능 — 안 본 데이터")
# (오른쪽) 무엇이 예측에 중요했나 — 상위 6개를 가로 막대로(아래→위로 클수록)
imp = pd.Series(rf.feature_importances_, index=X.columns).sort_values().tail(6)
ax[1].barh(range(len(imp)), imp.values, color="#59a14f")
ax[1].set_yticks(range(len(imp))); ax[1].set_yticklabels(imp.index, fontsize=9)
ax[1].set_xlabel("중요도"); ax[1].set_title("기대수명 예측에 중요한 변수 (상위 6)")
plt.tight_layout(); plt.show()"""),
    md("""---
...
plt.tight_layout(); plt.show()                                 # 여백 정리 후 화면에 표시"""),
    ...
    md("""### 🖼️ 같은 오류, 여러 그림 — "각도를 바꾸면 보인다"
산점도에서 튄 그 점은 **히스토그램·박스플롯에서도** 똑같이 드러난다. 한 각도로 안 보여도 다른 그림이 잡는다."""),
    code("""# 같은 오류를 '다른 그림'으로도 — 산점도 말고 히스토그램·박스플롯에서도 똑같이 튄다
fig, ax = plt.subplots(1, 2, figsize=(11, 4))                  # 그래프 2개를 가로로
# (왼쪽) 히스토그램 — 오류값은 단 1건(막대 높이 1)이라, 세로축을 로그로 해야 그 외딴 막대가 보인다
ax[0].hist(dirty["log_gdp"], bins=40, color="#e45756", edgecolor="white")
ax[0].set_yscale("log")                                        # 높이 1짜리 외딴 막대도 보이도록 로그 눈금
err = dirty["log_gdp"].max()                                   # 오류값(비정상적으로 큰 log소득)
ax[0].annotate("오류값", xy=(err, 1), xytext=(err - 3.5, 50),  # 그 외딴 막대를 화살표로 가리킴
               arrowprops=dict(arrowstyle="->"), ha="center")
ax[0].set_xlabel("log(1인당 GDP)"); ax[0].set_ylabel("국가-연도 수 (로그 눈금)")
ax[0].set_title("히스토그램 — 오른쪽 끝 외딴 막대")
# (오른쪽) 박스플롯 — 수염(정상 범위) 밖으로 동떨어진 점 하나(이상치)
ax[1].boxplot(dirty["log_gdp"].dropna(), showmeans=True)
ax[1].set_xticks([1]); ax[1].set_xticklabels(["전체"]); ax[1].set_ylabel("log(1인당 GDP)")
ax[1].set_title("박스플롯 — 수염 밖 유일한 이상치")
plt.tight_layout(); plt.show()"""),
